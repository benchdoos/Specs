...
import com.mmz.specs.application.managers.RecentItems;
...
public class ClientMainWindow extends JFrame {
    ...
    private JMenu getFileMenu() {
        ...
        openFileMenu.setIcon(new ImageIcon(Toolkit.getDefaultToolkit()
                .getImage(getClass().getResource("/img/gui/menu/open.png"))));
...
//                initFtp();
    ...
    private JMenu getRecentItemsMenu() {
        JMenu menu = new JMenu("Недавние файлы");
        menu.addMenuListener(new MenuListener() {
            @Override
            public void menuSelected(MenuEvent event) {
                RecentItems recentItems = RecentItems.getInstance();
                final List<String> items = recentItems.getItems();
                for (String path : items) {
                    JMenuItem item = new JMenuItem(path);
                    if (CommonUtils.getFileExtension(new File(path))
                            .equalsIgnoreCase(SupportedExtensionsConstants.EXPORT_TREE_EXTENSION)) {
                        item.setIcon(new ImageIcon(Toolkit.getDefaultToolkit()
                                .getImage(getClass().getResource("/img/gui/extensions/sptFileFormat.png"))));
                    }
                ...
                if (items.size() > 0) {
                    menu.addSeparator();
                    JMenuItem clear = new JMenuItem("Очистить список");
